...
> 🔴공격 에이전트가 6단계 킬체인을 구동하면, 🧠**DNN 탐지기+규칙**(감각·판정)이 위협을 탐지·분류하고
> 🔵**LLM(Claude) 방어 에이전트**(두뇌)가 대응을 지휘한다 — 탐지 신뢰도로 SLA(가용성) 대응강도를 선택하고,
> 판단이 애매한 구간에선 LLM이 2차 소견(자문)을 낸다.

![architecture](docs/architecture.png)

## 1. 핵심 요약
- **DNN+규칙 = 감각·판정(탐지·위협분류), LLM = 서술·지휘** — 분류·대응 선택은 DNN+규칙이 결정적으로 하고, LLM은 위협분석·지휘요약을 서술하며 애매구간엔 2차 소견(자문)을 낸다.
- **가용성(SLA) 인지 대응** — 탐지 신뢰도에 따라 대응 강도를 조절(고신뢰=강제 차단, 저신뢰=서비스 유지형 완화)해 자해성 서비스 중단을 줄인다.
- **적응형 공격 강건성** — 회피형(느린 표류) 스푸핑 대비 재학습(hardened) → 조기탐지율 0.60→0.95.
...
🔴 공격 에이전트 ─▶ 시나리오 시뮬레이터 ─▶ 🧠 DNN 탐지기 + 규칙 ─▶ 🔵 LLM 방어 에이전트 ─▶ NIST CSF 플레이북
   (6단계 킬체인)     (합성 신호/트래픽)      (위협분류·신뢰도·근거)   (SLA 대응지휘·서술, 애매시 2차소견) (대응→복구→예방)
```
...
| --- | --- |
| **Recon** | 대상 신호/인프라 식별 |
| **Attacker** | 6단계 킬체인 구동(회피형 프로파일 옵션 포함) — *전부 시뮬레이션·오프라인* |
| **Detector** | DNN 탐지기 호출 → 경보(위협·점수·신뢰도·탐지지연·증거·규칙근거) 생성. LLM이 위협분석 서술, 애매구간(DNN·규칙 불일치/경계 신뢰도)엔 **2차 소견(자문)** |
| **Responder** | 위협 분류 → MITRE ATT&CK/SPARTA 매핑 → NIST CSF 플레이북(대응/복구/예방) 선택. **탐지 신뢰도로 SLA 대응강도**(강제차단/서비스유지형완화) 선택 → 사고보고. LLM이 지휘요약 |
...
| 시나리오 | 지표 |
| --- | --- |
| A2 GNSS | 정확도 0.979 · 공격탐지 AUC 0.992 · TPR@0.5 0.950 · FPR@0.5 0.002 · 스푸핑 탐지지연(중앙값) ~4초 |
| B2 SATCOM | 이상탐지 AUC 0.984 · F1 0.958 · 탐지지연 ~2버킷(모뎀 마비 전 조기 탐지) |
| B3 CAN | 정확도 0.999 · macro-F1 0.999 (CAN IDS 문헌값과 부합) |
...
LLM 에이전트의 tool-calling 정식화(현재는 애매구간 2차 소견까지 구현), 온라인 스트리밍 탐지.
